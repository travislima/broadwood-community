...
| `photos` | Neighbourhood photos. Upload images to an `images/` folder in this repo (on GitHub: **Add file → Upload files**), then add one line per photo: `{ src: "images/name.jpg", caption: "..." }`. As soon as one photo is listed, a swipeable photo gallery replaces the illustration at the top of the page. Use real photos from the community — clean-ups, streets, parks, camera installations. Keep each under ~500 KB so the page stays fast. |
